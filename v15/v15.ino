...
SharpIR2 xxsR (sRight, 10, 99, 1080);
SharpIR2 xxsL (sLeft, 10, 99, 1080);
SharpIR2 xxsB (sBottom, 10, 99, 1080);
SharpIR2 xxsM (sMiddle, 10, 99, 1080);
SharpIR2 xxsT (sTop, 10, 99, 1080);

//double leftSensorValue, rightSensorValue, middleSensorValue, sideFSensor, sideBSensor;
//double leftSensorValueInt, rightSensorValueInt, middleSensorValueInt, sideFSensorInt, sideBSensorInt;
...
int xLxSensorValue, xRxSensorValue, xMxSensorValue, xSFxSensor, xSBxSensor;
int xLxSensorValueInt, xRxSensorValueInt, xMxSensorValueInt, xSFxSensorInt, xSBxSensorInt;
/**********************************************************************************************/
...
void loop()
{
  ...
  switch (userInput)
  {
    ...
    case 'b' :
      xxreadingSensors(); //reading all sensors (returning block distance)
      Serial.print(xLxSensorValueInt);
      Serial.print(xMxSensorValueInt);
      Serial.print(xRxSensorValueInt);
      Serial.print(xSFxSensorInt);
      Serial.print(xSBxSensorInt);
      Serial.println("");
      break;
...
/***************************** Block Function ****************************/
int block3Function(int original) // detect things 3 blocks or less
{
  int blocks;
...
int block2Function(int original) //detech things 2 blocks or less
{
  int blocks;
...
/********** TESTING NEW CODE - NOT FOR USE USE IN MAIN LOOP *****************************/
void xxloop()
{
  testcode();
  testcode2();
...
void testcode2()
{
  xxreadingSensors(); //reading all sensors (returning block distance)
  Serial.print("Left: ");
  Serial.print(xLxSensorValue);
  Serial.print(" | Middle: ");
  Serial.print(xMxSensorValue);
  Serial.print(" | Right: ");
  Serial.print(xRxSensorValue);
  Serial.print(" | sideF: ");
  Serial.print(xSFxSensor);
  Serial.print(" | sideB: ");
  Serial.print(xSBxSensor);
  Serial.println("");
}

void xxreadingSensors()
{
  for (int i = 5; i > 0; i--)
  {
    //    xxsensorSL [5 - i] = xxgetMedianDistance(xxsL);
    //    xxsensorSR [5 - i] = xxgetMedianDistance(xxsR);
    //    xxsensorSM [5 - i] = xxgetMedianDistance(xxsM);
    //    xxsensorSB [5 - i] = xxgetMedianDistance(xxsB);
    //    xxsensorST [5 - i] = xxgetMedianDistance(xxsT);

#define OFFSET 3
    xxsensorSL [5 - i] = xxsL.distance() + OFFSET;
    xxsensorSR [5 - i] = xxsR.distance() + OFFSET;
    xxsensorSM [5 - i] = xxsM.distance() + OFFSET;
    xxsensorSB [5 - i] = xxsB.distance() + OFFSET;
    xxsensorST [5 - i] = xxsT.distance() + OFFSET;
  }

  insertionSort(xxsensorSR);
  insertionSort(xxsensorSL);
  insertionSort(xxsensorSM);
  insertionSort(xxsensorSB);
  insertionSort(xxsensorST);

  xLxSensorValue = xxsensorSL[2];
  xRxSensorValue = xxsensorSR[2];
  xMxSensorValue = xxsensorSM[2];
  xSFxSensor = xxsensorSB[2];
  xSBxSensor = xxsensorST[2];

  xLxSensorValueInt = xx3BlockFunction(xLxSensorValue);
  xRxSensorValueInt = xx3BlockFunction(xRxSensorValue);
  xMxSensorValueInt = xx3BlockFunction(xMxSensorValue);
  xSFxSensorInt = xx3BlockFunction(xSFxSensor);
  xSBxSensorInt = xx3BlockFunction(xSBxSensor);
}

int xxgetMedianDistance(SharpIR2 sensor)
{
  RunningMedian sample = RunningMedian(10);
  for (int i = 0; i < 10; i ++)
  {
...
int xx2BlockFunction(int original)
{
  int blocks;
  blocks = original / 10;
...
int xx3BlockFunction(int original)
{
  int blocks;
  blocks = original / 10;
