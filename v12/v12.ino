...
void turnright()
{
  ...
  } while (count_m1 <= 380 || count_m2 <= 380);
...
void gostraightblock(int x)
{
  ...
  do
  {
    gostraight();
    delay(20);
  } while (count_m1 <= (250 * x) && count_m2 <= (250 * x));
...
void readingSensors()
{
  ...
  leftSensorValueInt = blockFunction(leftSensorValue);
  rightSensorValueInt = blockFunction(rightSensorValue);
  middleSensorValueInt = blockFunction(middleSensorValue);
...
{
  readingAlignSensors();
  while ((leftSensorValue != 11) && (rightSensorValue != 11) && (middleSensorValue != 11))
  {
    if ((leftSensorValue > 11) && (rightSensorValue > 11) && (middleSensorValue > 11))
    {
      md.setSpeeds (70 , 70);
    }

    else if ((leftSensorValue < 11) && (rightSensorValue < 11) && (middleSensorValue < 11))
    {
      md.setSpeeds (-70 , -70);
    }
    readingAlignSensors();
